...
function tone(status: string) { return status === "match" ? "success" as const : status === "material" ? "danger" as const : status === "missing" ? "warning" as const : "warning" as const; }
...
export default function ContractsPage() {
  ...
  return (
    <WorkspacePage eyebrow="Contract intelligence · working prototype" title="Il draft non vive in isolamento." description="Kalamos mette fianco a fianco i termini registrati durante l'acquisition e quelli estratti dal draft. Non approva contratti e non sostituisce il team legale: evidenzia cosa verificare." actions={<span className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-2.5 text-xs font-semibold text-amber-700">{review} elementi da verificare</span>}>
      ...
      <Panel title="Expected vs Contracted" subtitle="Ogni confronto conserva una fonte. In un deployment reale le fonti arriverebbero dai documenti e dalle note verificate della redazione.">
        <div className="divide-y divide-[#14213d]/7">
          {demoContract.terms.map((t) => (
            <div key={t.id} className="grid gap-5 px-5 py-5 sm:px-6 lg:grid-cols-[1fr_1fr_1fr_auto] lg:items-center">
              <div><div className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">Termine</div><div className="mt-1 font-serif text-lg font-semibold text-inchiostro">{t.label}</div></div>
              <div><div className="text-[10px] uppercase tracking-[0.12em] text-slate-400">Atteso</div><div className="mt-1 text-sm font-medium text-slate-700">{t.expected}</div><div className="mt-1 text-[11px] text-slate-400">{t.sourceExpected}</div></div>
              <div><div className="text-[10px] uppercase tracking-[0.12em] text-slate-400">Nel draft</div><div className="mt-1 text-sm font-medium text-slate-700">{t.contracted}</div><div className="mt-1 text-[11px] text-slate-400">{t.sourceContract}</div></div>
              <StatusPill tone={tone(t.status)}>{label(t.status)}</StatusPill>
            </div>
          ))}
        </div>
      </Panel>
